...
This vignette explains how to use `drep` to compute the Dengue reproduction from Force of Infection using real data. It will also show how to calculate the reprodution number under different assumptions regarding the relative infectiousness of the number of primary, secondary, tertiary and quaternary infections at endemic equlibrium.  
...
Extract the country age structure and have look at it. 
...
Lest's also define the lower and upper age limits (`l_lim` and `u_lim`) of the country age groups amnd the infectiousness weights (`phis`)  
...
Now we assume that only primary and secondary infections are infectious and therefore contribute to transmission. This implies that tertiary and secondary infections are not infectious.
