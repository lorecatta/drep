...
#'
#' @return A numeric vector of length equals \code{nrow(foi_data)}.
#'
#' @export
calculate_R0_foi_bulk <- function(foi_data,
  ...
  wrapper <- function(i) {
    n_j <-
      age_data[age_data[, id_0_field] == foi_data[i, id_0_field], 2:ncol(age_data)]

    FOI <- foi_data[i, foi_field]

    N <- foi_data[i, pop_field]

    calculate_R0(FOI, N, n_j, u_lim, l_lim, phis)
...
#'
#' @param phi_combs A list with different combinations of infectiousness
...
#'
#' @export
calculate_R0_infectiouss_bulk <-
  function(phi_combs, FOI, N, n_j, u_lim, l_lim) {
    wrapper <- function(i) {
      calculate_R0(
        FOI = FOI,
        N = N,
...
#'  combinations.
#'
#' @inheritParams calculate_R0_foi_bulk
#'
#' @inheritParams calculate_R0_infectiouss_bulk
#'
